...
describe "Users::Registrations" do
  describe "Registering", :js => true do
      ...
      click_link "Register"
      find('#new_user')
      find('#new_user').fill_in "user_email", with: new_user.email
			find('#new_user').fill_in "user_email_confirmation", with: new_user.email
			find('#new_user').fill_in "user_password", with: new_user.password
			find('#new_user').fill_in "user_pseudo", with: new_user.pseudo
  	end
  	it "should warn that email is empty" do
  		find('#new_user').fill_in "user_email", with: ""
  		find('#new_user').click_button "Submit"
  	...
  	# 	page.should have_selector "span.help-inline", text: 'Must be a valid e-mail adress.'
  	# end
